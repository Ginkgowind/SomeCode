                    ...
                    xtest_entry_func_t fini)
{
    ...
    snprintf(buf, sizeof(buf), "%s.%s", catagory, name); // 拼接catagory与name
...
void xtest_fail_message(const char *file, int lineno, const char *fmt, ...)
{
    ...
    va_start(ap, fmt);
    vsnprintf(msg, sizeof(msg) - 1, fmt, ap); // 格式化信息
    va_end(ap);
...
static int infoset_execute(entry_infos_st *infos, int argc, char **argv)
{
    ...
    qsort(infos->infos, infos->ninfo, sizeof(xtest_entry_info_st *), casecmp); // 排序

    export_header(infos); // 输出测试起始的信息
    ...
    for (i = 0; i < infos->ninfo; ++i)
    {
        ...
        export_case_header(ent->name, ent->file, ent->lineno); // EXP_MODE_XML 使用

        if (!s_fork) // 若不fork，就在当前进程执行
        {
            if ((void *)ent->init)
                ent->init();
        ...
        export_case_tailer(); // EXP_MODE_XML 使用
    }
    export_tailer(); // EXP_MODE_XML 使用
    return 0;
}
